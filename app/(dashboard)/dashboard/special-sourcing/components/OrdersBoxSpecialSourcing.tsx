...
const ComponentsAccordionsBasic = () => {
  ...
  const visibleOrders = useMemo(
    () =>
      orderALL.filter((order: any) =>
        matchesServiceOrderSearch(deferredSearchQuery, [
          order.pidSpecialSourcing,
          order.pidUser,
          order.productName,
          order.whatsappNumber,
          order.productQualityRatings,
          order.targetUnitPrice,
  ...
  async function fetchDataOrder() {
    try {
      ...
      const res = await fetch(
        `/api/get-data/special-sourcing-many?status=${status}`,
      );
  ...
  return (
    <>
      ...
      <ServiceOrderSearch
        ...
        placeholder="Search this stage by request ID, product, WhatsApp or target price…"
      />
      {visibleOrders.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-muted/5 px-4 py-12 text-center text-sm font-semibold text-muted-foreground">
          No sourcing requests match your search in this stage.
        </div>
      ) : null}
        ...
        return (
          <div className="mb-5" key={datax.pidSpecialSourcing || index}>
            <div className="space-y-2 font-semibold">
              <div
                className="rounded border border-[#d3d3d3] dark:border-[#1b2e4b]"
                key={index + 1}
              >
                <button
                  type="button"
                  className={`flex w-full items-center p-4 text-white-dark dark:bg-[#1b2e4b] ${active === `${index + 1}` ? "!text-primary" : ""}`}
                  onClick={() => togglePara(`${index + 1}`)}
                >
                  <b className="text-xl">
                    #{index + 1} : {datax.productName}
                  </b>{" "}
                  &nbsp; | ORDER ID: {datax.pidSpecialSourcing}
                  <div
                    className={`ltr:ml-auto rtl:mr-auto ${active === `${index + 1}` ? "rotate-180" : ""}`}
                  >
                    <BookDown />
                  </div>
                  <br />
                      ...
                      <div className="p-7 flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
                        <div className="w-full md:w-1/2">
                          <button
                            ...
                            className="w-full btn btn-dark mt-4 bg-gray-700 dark:bg-gray-600 text-white py-3 rounded-md text-sm shadow hover:bg-gray-800 dark:hover:bg-gray-700"
                          >
                        ...
                        <div className="w-full md:w-1/2">
                          <button
                            ...
                            className="btn btn-secondary mt-4 w-full bg-indigo-600 dark:bg-indigo-500 text-white py-3 rounded-md text-sm shadow hover:bg-indigo-700 dark:hover:bg-indigo-600"
                          >
                            APPROVE
                          </button>
                          <small>
                            Approve this Order for further processing
                          </small>
                        </div>
                      </div>
                    </form>
