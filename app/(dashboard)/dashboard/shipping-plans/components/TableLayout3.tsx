...
//       value.toString().toLowerCase().includes(searchTerm.toLowerCase()) as any
//     )
//   );

  // Filter data based on search term
  const filteredData = data?.filter((item) => // Use optional chaining
    Object.values(item).some((value) =>
  ...
  const handleAction = (action: string, item: TableData) => {
    alert(`${action} clicked for ${item.countryName}`);
  ...
  const exportToPDF = () => {
    ...
    doc.autoTable({
      head: [['ID', 'Country']],
      body: filteredData.map((item) => [item.id, item.countryName,]),
    });
    doc.save('table.pdf');
  };
  ...
  return (
    <div className="">
      <div className="max-w-6xlHIDEME mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
        <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Countries Shipping Plan & Rates</h1>
            ...
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th
                  ...
                  className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider cursor-pointer"
                >
                  country {sortConfig?.key === 'countryName' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                </th>
