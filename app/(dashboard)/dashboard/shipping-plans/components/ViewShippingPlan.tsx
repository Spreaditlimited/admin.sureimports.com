...
export default function ProductsTable() {
  const [products, setUProducts] = useState<ProductProps[]>([]);
  ...
  const getProducts = async (search: string, page: number) => {
            ...
            try {
                ...
                const response = await fetch(`/api/get-records/products?search=${search}&page=${page}&limit=5`);
                const data = await response.json();
                setUProducts(data.products);
...
<div>
                <button type="button" onClick={() => { router.push('/dashboard/products/add');}} className="btn btn-dark w-full"><MdAddToPhotos /> &nbsp; Add New Product</button>
            </div>
    <h1 className="text-base font-bold m-2 p-3">View Products Records</h1>
      {/* Search Input */}
      <input
        type="text"
        value={search}
      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th scope="col" className="px-6 py-3">Price (NGN)</th>
        ...
        {products.length ? (
          ...
          <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td className="px-6 py-4">{product.productMOQ}</td>
            <td className="px-6 py-4">{product.pidCategory}</td>
            <td className="px-6 py-4">
