...
const OrdersBoxShippingOnly = () => {
  ...
  const visibleOrders = useMemo(
    () =>
      orderALL.filter((order) =>
        matchesServiceOrderSearch(deferredSearchQuery, [
          order.pidShippingOnly,
          order.pidUser,
          ...
          order.expectedShipments,
          order.description,
          order.invoice?.invoiceNumber,
          order.invoice?.pidInvoice,
          order.status,
        ]),
      ),
    [deferredSearchQuery, orderALL],
  );
  ...
  const fetchDataOrder = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/get-data/shipping-only-many?status=${status}`,
      );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("shipping-only-request-created", refresh);
  }, [fetchDataOrder]);
  ...
  const getNextStatusForAction = (
    ...
    return (
      getShippingOnlyNextStatus(currentStatus, action, isInternational) || ""
    );
  };
    ...
    try {
      ...
      if (data?.responsex?.status === "SUCCESS") {
        ...
        toast.error(
          data?.responsex?.message ||
            data?.message ||
            "Unable to update freight state.",
        );
      }
    } catch {
  ...
  const handleIssueInvoice = async (pidInvoice: string) => {
    try {
      ...
      const res = await fetch(
        `/api/invoicing/invoices/${encodeURIComponent(pidInvoice)}/issue`,
        {
          method: "POST",
        },
      );
      ...
      if (!res.ok) {
        toast.error(
          data?.message || data?.statusx || "Failed to issue invoice.",
        );
        return;
      }
  ...
  if (loading)
    return (
      <div className="py-20 flex justify-center">
        <RefreshCw className="animate-spin text-primary w-8 h-8 opacity-20" />
      </div>
    );
    ...
    return (
      <div className="py-24 text-center border-2 border-dashed border-border rounded-xl bg-muted/5 mx-2">
        ...
        <p className="text-sm font-bold uppercase tracking-widest text-muted-foreground italic">
          No {getShippingOnlyStatusLabel(status)} records found
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex items-center gap-3 px-1 mb-6">
        ...
        <div>
          <h2 className="text-sm font-bold text-foreground uppercase tracking-widest">
            Freight Queue: {getShippingOnlyStatusLabel(status)}
          </h2>
      ...
      <ServiceOrderSearch
        ...
        totalCount={orderALL.length}
        placeholder="Search this stage by request ID, WhatsApp, destination, tracking or invoice…"
      />
      {visibleOrders.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-muted/5 px-4 py-12 text-center text-sm font-semibold text-muted-foreground">
          No shipping requests match your search in this stage.
        </div>
      ) : null}
      ...
      {visibleOrders.map((order, index) => {
        ...
        const nextApproveStatus = getNextStatusForAction(
          order.status,
          "approve",
          isInternational,
        );
        const approveActionLabel =
          getActionLabelForNextStatus(nextApproveStatus);
        ...
        const canCancel =
          currentStatus !== "request-cancelled" &&
          !hasProductShipped(currentStatus, isInternational);

        return (
          <div
            key={order.pidShippingOnly}
            className="bg-card border border-border rounded-xl shadow-soft overflow-hidden transition-all duration-300"
          >
            {/* Header Row */}
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-4">
                <span className="text-xs font-bold font-mono text-muted-foreground opacity-50">
                  #{index + 1}
                </span>
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="px-2 py-0.5 rounded-md bg-muted text-[9px] font-bold uppercase tracking-widest border border-border">
                      ID: {order.pidShippingOnly}
                    </div>
                  </div>
                  <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground font-medium uppercase tracking-tight">
                    ...
                    <span className="flex items-center gap-1">
                      <Globe className="w-3 h-3" /> Shipping To:{" "}
                      {formatReadable(destinationName)}
                    </span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4 mt-4 md:mt-0">
                ...
                <div
                  className={`p-2 rounded-lg transition-transform duration-300 ${active === order.pidShippingOnly ? "rotate-180 bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground"}`}
                >
            ...
            <AnimateHeight
              duration={300}
              height={active === order.pidShippingOnly ? "auto" : 0}
            >
              <div className="border-t border-border bg-muted/10">
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-primary flex items-center gap-2">
                      <Truck className="w-3.5 h-3.5" /> Logistics Specs
                    </h4>
                    <div className="space-y-2.5">
                      {[
                        {
                          label: "Gross Weight",
                          val: order.grossWeight,
                          icon: Scale,
                        },
                        {
                          label: "Shipping Plan",
                          val: formatShippingPlanDisplay(
                            order.shippingPlanName || order.shippingPlan,
                          ),
                          icon: Clock,
                        },
                        {
                          label: "Expected Shipments",
                          val: order.expectedShipments || "Standard",
                          icon: Calendar,
                        },
                      ].map((item, i) => (
                        <div
                  ...
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-primary flex items-center gap-2">
                      <ShieldCheck className="w-3.5 h-3.5" /> Handling Prefs
                    </h4>
                    <div className="space-y-2">
                      {[
                        {
                          label: "Want Product Verification",
                          val: order.wantProductVerification,
                        },
                        {
                          label: "Want Consolidation",
                          val: order.wantConsolidation,
                        },
                        {
                          label: "Multiple Suppliers",
                          val: order.multipleSuppliers,
                        },
                      ].map((item, i) => (
                        <div
                          key={i}
                          className="flex items-center justify-between p-2 rounded border text-[10px] uppercase font-bold tracking-tight bg-muted/50 border-border text-muted-foreground"
                        >
                          <span>{item.label}</span>
                          <span className="text-foreground">
                            {formatSelection(item.val)}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-primary flex items-center gap-2">
                      <MessageSquare className="w-3.5 h-3.5" /> Agent Briefing
                    </h4>
                    <div className="p-3 rounded-lg bg-card border border-border text-[11px] leading-relaxed text-muted-foreground italic h-[110px] overflow-y-auto custom-scrollbar">
                      &quot;
                      {order.description ||
                        "No additional instructions provided by the user."}
                      &quot;
                    </div>
                  </div>
                </div>
                ...
                <form
                  ...
                  className="p-6 bg-muted/30 border-t border-border"
                >
                      ...
                      {isCompleted ? (
                        ...
                        <button
                          ...
                          disabled={Boolean(
                            order.invoice &&
                              invoiceStatus === "DRAFT" &&
                              issuingInvoiceId === order.invoice.pidInvoice,
                          )}
                          onClick={() => {
                      ...
                      {!isCompleted && (
                        <p className="text-[9px] text-center text-muted-foreground uppercase font-bold tracking-tighter">
                          ...
                          <span className="text-primary">
                            {getShippingOnlyStatusLabel(nextApproveStatus)}
                          </span>
                        </p>
                      )}
                    </div>
          ...
          <div className="mx-4 w-full max-w-md overflow-hidden rounded-xl bg-card border border-border shadow-soft animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6">
              ...
              <p className="mb-4 text-center text-sm text-muted-foreground">
                This will move the request to Request Cancelled and notify the
                customer.
              </p>
              ...
              <div className="mb-6 rounded-md border border-border bg-muted/30 p-4 text-sm text-foreground shadow-inner">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ID: {cancelTarget.pidShippingOnly}
                </p>
              </div>
                ...
                <button
                  ...
                  onClick={() => {
                    const destinationName =
                      cancelTarget.shippingToName || cancelTarget.shippingTo;
                    processStatusChange({
