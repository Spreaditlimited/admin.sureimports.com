...
const ComponentsAccordionsBasic = () => {
    ...
    () =>
      orderALL.filter((order) =>
        matchesServiceOrderSearch(deferredSearchQuery, [
          order.pidPaySmallSmall,
          order.pidUser,
          order.pidProduct,
          order.store?.productName,
          order.productName,
          order.store?.productDescription,
          order.productDescription,
          order.users?.userFirstname,
          order.users?.userLastname,
          order.users?.userEmail,
          order.users?.userPhone,
          order.users?.phone,
          order.amount,
          order.quantity,
          order.status,
  ...
  async function fetchDataOrder() {
    try {
      const res = await fetch(
        `/api/get-data/pay-small-small-many?status=${status}`,
      );
      if (res.ok) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await fetch(
        "/api/status-processing/pay-small-small?status=" + status,
        {
          method: "POST",
          body: formData,
        },
      ...
      if (data.responsex?.status === "SUCCESS") {
        navigateWithAlert(
          "/dashboard",
          "success",
          "Payment details successfully submitted. Awaiting confirmation.",
        );
      } else if (
        data.responsex?.status === "ACTION_FAILED" ||
        data.responsex?.status === "EMPTY_BANK_PAYMENT_DETAILS"
      ) {
  ...
  return (
    <div className="space-y-4">
      ...
      <ServiceOrderSearch
        ...
        placeholder="Search this stage by order ID, customer, phone, product or amount…"
      />
      {visibleOrders.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-muted/5 px-4 py-12 text-center text-sm font-semibold text-muted-foreground">
          No instalment orders match your search in this stage.
        </div>
      ) : null}
                ...
                <div className="bg-card border border-border rounded-lg p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4">
                    <DetailItem
                      label="Full Name"
                      value={
                        `${datax.users?.userFirstname || ""} ${datax.users?.userLastname || ""}`.trim() ||
                        "N/A"
                      }
                    />
                    <DetailItem
