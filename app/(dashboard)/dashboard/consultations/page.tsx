...
export default async function ConsultationsAdminPage() {
  ...
  const upcomingCount = bookings.filter(
    (booking) => booking.status === 'booked' && booking.slotStartUtc >= new Date(),
  ).length;
  const pendingPaymentCount = bookings.filter(
    (booking) => booking.status === 'pending_payment',
  ).length;
  const failedCount = bookings.filter((booking) => booking.status.includes('failed')).length;
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          { label: 'Upcoming booked calls', value: upcomingCount, icon: CalendarClock },
          { label: 'Pending payment', value: pendingPaymentCount, icon: Clock },
          { label: 'Payment/Zoom issues', value: failedCount, icon: XCircle },
        ].map((stat) => (
      ...
      {bookings.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-card p-20 text-center shadow-soft">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Paid consultation bookings will appear here after customers start checkout.
          </p>
        </div>
      ) : (
              ...
              <div className="border-b border-border p-5 sm:p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {formatDate(booking.slotStartUtc)} • {booking.durationMinutes} minutes
                    </p>
                  </div>
