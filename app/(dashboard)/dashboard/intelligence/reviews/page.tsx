...
function statusClass(status: string) {
  if (status === 'answered') return 'border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300';
  if (status === 'in_review') return 'border-blue-500/20 bg-blue-500/10 text-blue-700';
  if (status === 'closed') return 'border-slate-500/20 bg-slate-100 text-slate-700';
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <h2 className="mt-3 text-lg font-bold text-foreground">
                      {request.supplierName || request.nicheName || 'Supplier review request'}
                    </h2>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {request.requestType.replace(/_/g, ' ')} • {formatDate(request.createdAt)}
                    </p>
                  </div>
              ...
              <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1fr_420px]">
                <div className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <Info label="Supplier address in Chinese" value={request.supplierAddress} />
                ...
                >
                  ...
                  {request.status !== 'answered' ? (
                    <p className="mt-3 flex gap-2 text-xs leading-relaxed text-muted-foreground">
                      ...
                      Customer email is sent only when status is saved as
                      Answered.
                    </p>
                  ) : null}
                </form>
