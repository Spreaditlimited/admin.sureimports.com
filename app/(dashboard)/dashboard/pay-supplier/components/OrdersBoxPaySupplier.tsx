    ...
    () =>
      orderALL.filter((order) =>
        matchesServiceOrderSearch(deferredSearchQuery, [
          order.pidPaySupplier,
          order.pidUser,
          order.supplierName,
          order.supplierPhone,
          order.supplierEmail,
          order.supplierBankAccountDetails,
          order.amountToPayInYuan,
          order.amountToPayInNaira,
          order.status,
        ]),
      ),
  ...
  async function fetchDataOrder() {
    try {
      ...
      const res = await fetch(
        `/api/get-data/pay-supplier-many?status=${status}`,
      );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.statusx == "SUCCESS") {
        navigateWithAlert(
          "/dashboard/pay-supplier?status=paid-supplier",
          "success",
          "Payment details was successfully confirmed and marked paid.",
        );
      }
      ...
      if (data.statusx == "CANCELLED") {
        navigateWithAlert(
          "/dashboard/pay-supplier?status=request-cancelled",
          "success",
          "Pay Supplier Order has been successfully cancelled.",
        );
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-4">
      ...
      <ServiceOrderSearch
        ...
        placeholder="Search this stage by request ID, supplier, phone, email or amount…"
      />
      {visibleOrders.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-muted/5 px-4 py-12 text-center text-sm font-semibold text-muted-foreground">
          No supplier-payment requests match your search in this stage.
        </div>
      ) : null}

      {visibleOrders.map((datax: any, index: number) => {
          ...
          >
            ...
            >
              <div className="flex flex-col gap-1 mb-3 sm:mb-0">
                <span
                  className={`text-base font-bold ${isActive ? "text-primary" : "text-foreground"}`}
                >
              ...
              <div className="flex items-center justify-between sm:justify-end gap-6">
                <div className="flex flex-col sm:items-end text-xs text-muted-foreground">
                  <span>
                    Updated:{" "}
                    {formatPaySupplierTime(datax.updatedAt || datax.createdAt)}
                  </span>
                  <span>Created: {formatPaySupplierTime(datax.createdAt)}</span>
                </div>
                <div
                  className={`text-muted-foreground transition-transform duration-300 ${isActive ? "rotate-180 text-primary" : ""}`}
                >
              ...
              <AnimateHeight duration={300} height={isActive ? "auto" : 0}>
                <div className="border-t border-border p-4 sm:p-6 bg-background/50 space-y-6">
                  <div className="bg-card border border-border rounded-lg p-5 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-y-6 gap-x-4 text-sm">
                      <DetailItem
                        label="Request ID"
                        value={datax.pidPaySupplier || "N/A"}
                      />
                      <DetailItem
                        label="Supplier Name"
                        value={datax.supplierName || "N/A"}
                      />
                      <DetailItem
                        label="Supplier Number"
                        value={datax.supplierPhone || "N/A"}
                      />
                      <DetailItem
                        label="Supplier Email"
                        value={datax.supplierEmail || "N/A"}
                      />
                      <div className="sm:col-span-2 xl:col-span-4">
                        <DetailItem
                          label="Supplier Bank Details"
                          value={datax.supplierBankAccountDetails || "N/A"}
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-5 mt-5 border-t border-border">
                      <DetailItem
                        label="Amount to pay in Yuan"
                        value={`¥${
                          ((datax.amountToPayInYuan as number) / 1)
                            .toFixed(2)
                            .toString()
                      ...
                      />
                      <DetailItem
                        label="Amount to pay in Naira"
                        value={`₦${
                          ((datax.amountToPayInNaira as number) / 1)
                            .toFixed(2)
                            .toString()
                            .replace(/\B(?=(\d{3})+(?!\d))/g, ",") as string
                        }`}
                ...
                >
                  <input
                    ...
                    id="pidUser"
                    name="pidUser"
                    defaultValue={datax.pidUser}
                  />
                  <input
                    type="hidden"
                    id="pidOrder"
                    name="pidOrder"
                    defaultValue={datax.pidPaySupplier}
                  />
                      ...
                      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-5 pt-0">
                        <div className="w-full md:w-1/1">
                          <button
                            ...
                            className="w-full bg-primary text-primary-foreground hover:bg-primary/90 py-2.5 px-4 rounded-md text-sm font-medium transition-colors shadow-sm"
                          >
                            Send Message
                          </button>
                      ...
                      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-5 pt-0">
                        <div className="w-full md:w-1/2">
                          <button
                            ...
                            onClick={() => setActionType("request-cancelled")}
                            className="w-full rounded-md bg-destructive px-4 py-2.5 text-sm font-medium text-destructive-foreground transition-colors hover:bg-destructive/90 shadow-sm"
                          >
                            Cancel Request
                          </button>
                          <small className="mt-2 block text-xs text-muted-foreground">
                            Cancel this request if there are issues
                          </small>
                        </div>
                        ...
                        <div className="w-full md:w-1/2">
                          <button
                            ...
                            name="action"
                            value="paid-supplier"
                            onClick={() => setActionType("paid-supplier")}
                            className="w-full bg-primary text-primary-foreground hover:bg-primary/90 py-2.5 px-4 rounded-md text-sm font-medium transition-colors shadow-sm"
                          >
                            Mark Paid
                          </button>
                          <small className="mt-2 block text-xs text-muted-foreground">
                            Mark this supplier payment as paid
                          </small>
                        </div>
                      </div>
                    </>
