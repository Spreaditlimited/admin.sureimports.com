...
export const AGREEMENT_VERSION = 'partner-2026-09-12-v3';
...
export function agreementOffer(business: AgreementBusiness) {
  if (business.country !== 'NG' || business.settlementCurrency !== 'NGN' ||
  ...
  const sections = [
    { title: "Our agreement", text: "This agreement sets out the responsibilities of your business and Sure Importers Limited. Read it together with the commercial schedule shown here. Your acceptance is recorded against this version and schedule. Business verification is completed automatically when you accept this agreement after business-fit approval and document verification. Acceptance activates your partner account, enables storefront customization and payment collection, and requires no further admin approval. Storefront publication remains a separate setup step." },
    { title: 'Your business and authority', text: `This agreement is between Sure Importers Limited and ${business.legalName} (registration ${business.registrationNumber}), acting through its authorised representative. For a registered business name, the representative acts for the relevant proprietor or proprietors. The programme is for registered Nigerian businesses. You confirm that you are authorised to accept these terms for this business.` },
    ...
    { title: 'Service charge and earnings', text: `The service charge is ${business.serviceChargeBps / 100}% of product cost. Your allocation is ${business.partnerShareBps / 100}% of product cost; Sure Imports retains ${(business.serviceChargeBps - business.partnerShareBps) / 100}%. These percentages apply to product cost, not to the service charge itself. Product purchase funds and shipping charges are not your earnings. Partners earn no shipping commission. Additional platform or hosting charges must be disclosed and agreed separately.` },
    { title: "Payments and settlement", text: "Payments are collected in NGN through the authorised Paystack checkout. Applicable processing fees are disclosed to and borne by the customer, subject to applicable requirements. Payments outside the authorised checkout are not part of this flow. Checkout may display Sure Imports branding. Sure Imports receives the customer payment without an automatic partner split. Your allocated earnings are recorded as Pending in your earnings wallet. They become Available only when your customer confirms delivery or pickup of the goods; delivery to your business alone does not release earnings. Disputed amounts remain On hold pending review. You may request withdrawal of Available earnings to your verified bank account. Sure Imports processes withdrawals through Paystack Transfers from its funded balance. A payout is recorded as Paid only after provider confirmation; processing times and transfer failures may delay receipt. Refunds and reversals adjust the corresponding earnings, including amounts already paid, and any resulting shortfall must be resolved before further withdrawal. This is an earnings ledger, not a deposit account or escrow service." },
...
export function agreementWorkflow(business: AgreementBusiness & { status: string }, kyc: { status: string; revision: number } | null, review: { businessFit?: { decision?: string; hardBlockersCleared?: boolean } }, receipt: AgreementReceipt | null, confirmation: { reference: string; confirmedAt: string } | null) {
  const enabled = PARTNER_WALLET_READY && ['PENDING', 'ACTIVE'].includes(business.status) && kyc?.status === 'VERIFIED' && review.businessFit?.decision === 'PILOT_APPROVED' && review.businessFit.hardBlockersCleared === true;
