...
const CATEGORY_TERMS: Record<string, string[]> = {
  ...
  "high-quality-sneakers-with-oem-capability": [
    "sneaker",
    "shoe",
    "footwear",
    "trainer",
...
export function validateReportQuality(
  ...
  if (snapshot.suppliers.length < 10) {
    errors.push(
      `requires at least 10 verified manufacturers; found ${snapshot.suppliers.length}`,
    );
  }
  ...
  if (!options.skipCoverCheck) {
    const imagePath = coverPath(product);
    const imageBytes = options.coverImageBytes;
    if (typeof imageBytes === "number") {
      if (imageBytes < 150_000) {
        errors.push(
          "category-specific cover image is below the production-quality threshold",
        );
      }
  ...
  snapshot.suppliers.forEach((supplier, index) => {
    ...
    const site = supplier.officialWebsite
      .trim()
      .toLowerCase()
      .replace(/\/$/, "");
    if (!name || names.has(name))
      errors.push(`${ref}: duplicate or missing manufacturer name`);
    if (!site || sites.has(site))
      errors.push(`${ref}: duplicate or missing official website`);
    names.add(name);
    sites.add(site);

    if (supplier.productFit.trim().length < 35)
      errors.push(`${ref}: product-fit assessment is too thin`);
    if (supplier.productsMade.filter(Boolean).length < 2)
      errors.push(`${ref}: fewer than two specific product categories`);
    if (supplier.buyerNotes.trim().length < 55)
      errors.push(`${ref}: Sure Imports assessment is too thin`);
    if (!/^https?:\/\//i.test(supplier.officialWebsite))
      errors.push(`${ref}: invalid official website`);
    if (supplier.whatsapp.replace(/\D/g, "").length < 8)
      errors.push(`${ref}: attributable WhatsApp route is missing`);
    if (!supplier.lastVerifiedAt)
      errors.push(`${ref}: verification date is missing`);

    const text = supplierText(supplier);
    if (
      relevanceTerms.length &&
      !relevanceTerms.some((term) => text.includes(term))
    ) {
      errors.push(
        `${ref}: product evidence does not match ${snapshot.name} (reviewed: ${text.slice(0, 180)})`,
      );
    }
    const weak = WEAK_LANGUAGE.find((phrase) => text.includes(phrase));
    if (weak)
      errors.push(`${ref}: weak or uncertain language detected (${weak})`);
  });
