...
interface WalletCreditEmailData {
  userEmail: string;
  userName: string;
  amount: number;
...
export default async function sendWalletCreditEmail(
...
<table style="width:100%;border-collapse:collapse;margin-top:6px;border:1px solid #e5e7eb;">
  <tr><td style="padding:8px;border:1px solid #e5e7eb;background:#f8fafc;"><b>Amount Credited</b></td><td style="padding:8px;border:1px solid #e5e7eb;">${formatAmount(data.amount)}</td></tr>
  ...
  ${balanceRow}
  <tr><td style="padding:8px;border:1px solid #e5e7eb;background:#f8fafc;"><b>Date</b></td><td style="padding:8px;border:1px solid #e5e7eb;">${data.creditedAt}</td></tr>
</table>`;

    await xMail({
      xEmail: data.userEmail,
      xTitle: `Wallet Credit Notice - ${formatAmount(data.amount)}`,
      xBodyTitle: 'Wallet Credit Notice',
      xBody1: `Hello ${data.userName},<br />A credit has been applied to your Sure Imports wallet.`,
      xBody2: `${table}<br />If you need clarification on this credit, please contact Sure Imports support with the reference above.`,
    ...
    console.error('Wallet credit email failed:', error);
    return false;
  }
}
