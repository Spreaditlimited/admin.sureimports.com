...
export async function getBodyCameraAdminAccess() {
  ...
  const admin = await prisma.admin.findUnique({
    ...
    select: {
      ...
      userStatus: true,
      permissions: {
        where: { serviceKey: 'body_camera_solutions' },
...
export async function requireBodyCameraEditAccess() {
  const admin = await getBodyCameraAdminAccess();
